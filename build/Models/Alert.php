<?php

declare(strict_types=1);

namespace ApeDevDe\MicrosoftGraphSdk\Models;

/**
 * Alert
 */
class Alert
{
    /** @var array<string, mixed> Raw data from API response */
    private array $rawData = [];

    /** The unique identifier for an entity. Read-only. */
    public ?string $id = null;

    /** Name or alias of the activity group (attacker) this alert is attributed to. */
    public ?string $activityGroupName = null;

    /** 
     * 
     * @var AlertDetection[]
     */
    public array $alertDetections = [];

    /** Name of the analyst the alert is assigned to for triage, investigation, or remediation (supports update). */
    public ?string $assignedTo = null;

    /** Azure subscription ID, present if this alert is related to an Azure resource. */
    public ?string $azureSubscriptionId = null;

    /** Microsoft Entra tenant ID. Required. */
    public ?string $azureTenantId = null;

    /** Category of the alert (for example, credentialTheft, ransomware). */
    public ?string $category = null;

    /** Time at which the alert was closed. The Timestamp type represents date and time information using ISO 8601 format and is always in UTC time. For example, midnight UTC on Jan 1, 2014 is 2014-01-01T00:00:00Z (supports update). */
    public ?\DateTimeInterface $closedDateTime = null;

    /** 
     * Security-related stateful information generated by the provider about the cloud application/s related to this alert.
     * @var CloudAppSecurityState[]
     */
    public array $cloudAppStates = [];

    /** 
     * Customer-provided comments on alert (for customer alert management) (supports update).
     * @var string[]
     */
    public array $comments = [];

    /** Confidence of the detection logic (percentage between 1-100). */
    public ?float $confidence = null;

    /** Time at which the alert was created by the alert provider. The Timestamp type represents date and time information using ISO 8601 format and is always in UTC time. For example, midnight UTC on Jan 1, 2014 is 2014-01-01T00:00:00Z. Required. */
    public ?\DateTimeInterface $createdDateTime = null;

    /** Alert description. */
    public ?string $description = null;

    /** 
     * Set of alerts related to this alert entity (each alert is pushed to the SIEM as a separate record).
     * @var string[]
     */
    public array $detectionIds = [];

    /** Time at which the event or events that served as the trigger to generate the alert occurred. The Timestamp type represents date and time information using ISO 8601 format and is always in UTC time. For example, midnight UTC on Jan 1, 2014 is 2014-01-01T00:00:00Z. Required. */
    public ?\DateTimeInterface $eventDateTime = null;

    /** 
     * Analyst feedback on the alert. Possible values are: unknown, truePositive, falsePositive, benignPositive. Supports update.
     * @var AlertFeedback|\stdClass|null
     */
    public AlertFeedback|\stdClass|null $feedback = null;

    /** 
     * Security-related stateful information generated by the provider about the file(s) related to this alert.
     * @var FileSecurityState[]
     */
    public array $fileStates = [];

    /** 
     * 
     * @var AlertHistoryState[]
     */
    public array $historyStates = [];

    /** 
     * Security-related stateful information generated by the provider about the host(s) related to this alert.
     * @var HostSecurityState[]
     */
    public array $hostStates = [];

    /** 
     * IDs of incidents related to current alert.
     * @var string[]
     */
    public array $incidentIds = [];

    /** 
     * 
     * @var InvestigationSecurityState[]
     */
    public array $investigationSecurityStates = [];

    /**  */
    public ?\DateTimeInterface $lastEventDateTime = null;

    /** Time at which the alert entity was last modified. The Timestamp type represents date and time information using ISO 8601 format and is always in UTC time. For example, midnight UTC on Jan 1, 2014 is 2014-01-01T00:00:00Z. */
    public ?\DateTimeInterface $lastModifiedDateTime = null;

    /** 
     * Threat Intelligence pertaining to malware related to this alert.
     * @var MalwareState[]
     */
    public array $malwareStates = [];

    /** 
     * 
     * @var MessageSecurityState[]
     */
    public array $messageSecurityStates = [];

    /** 
     * Security-related stateful information generated by the provider about the network connection(s) related to this alert.
     * @var NetworkConnection[]
     */
    public array $networkConnections = [];

    /** 
     * Security-related stateful information generated by the provider about the process or processes related to this alert.
     * @var Process[]
     */
    public array $processes = [];

    /** 
     * Vendor/provider recommended action(s) to take as a result of the alert (for example, isolate machine, enforce2FA, reimage host).
     * @var string[]
     */
    public array $recommendedActions = [];

    /** 
     * Security-related stateful information generated by the provider about the registry keys related to this alert.
     * @var RegistryKeyState[]
     */
    public array $registryKeyStates = [];

    /** 
     * Resources related to current alert. For example, for some alerts this can have the Azure Resource value.
     * @var SecurityResource[]
     */
    public array $securityResources = [];

    /**  */
    public ?AlertSeverity $severity = null;

    /** 
     * Hyperlinks (URIs) to the source material related to the alert, for example, provider's user interface for alerts or log search.
     * @var string[]
     */
    public array $sourceMaterials = [];

    /**  */
    public ?AlertStatus $status = null;

    /** 
     * User-definable labels that can be applied to an alert and can serve as filter conditions (for example 'HVA', 'SAW') (supports update).
     * @var string[]
     */
    public array $tags = [];

    /** Alert title. Required. */
    public ?string $title = null;

    /** 
     * Security-related information about the specific properties that triggered the alert (properties appearing in the alert). Alerts might contain information about multiple users, hosts, files, ip addresses. This field indicates which properties triggered the alert generation.
     * @var AlertTrigger[]
     */
    public array $triggers = [];

    /** 
     * 
     * @var UriClickSecurityState[]
     */
    public array $uriClickSecurityStates = [];

    /** 
     * Security-related stateful information generated by the provider about the user accounts related to this alert.
     * @var UserSecurityState[]
     */
    public array $userStates = [];

    /** 
     * Complex type containing details about the security product/service vendor, provider, and subprovider (for example, vendor=Microsoft; provider=Windows Defender ATP; subProvider=AppLocker). Required.
     * @var SecurityVendorInformation|\stdClass|null
     */
    public SecurityVendorInformation|\stdClass|null $vendorInformation = null;

    /** 
     * Threat intelligence pertaining to one or more vulnerabilities related to this alert.
     * @var VulnerabilityState[]
     */
    public array $vulnerabilityStates = [];


    /**
     * Constructor - fast deserialization
     * @param array<string, mixed> $data
     */
    public function __construct(array $data = [])
    {
        $this->rawData = $data;
        if (isset($data['id'])) {
            $this->id = $data['id'];
        }
        if (isset($data['activityGroupName'])) {
            $this->activityGroupName = $data['activityGroupName'];
        }
        if (isset($data['alertDetections'])) {
            $this->alertDetections = $data['alertDetections'];
        }
        if (isset($data['assignedTo'])) {
            $this->assignedTo = $data['assignedTo'];
        }
        if (isset($data['azureSubscriptionId'])) {
            $this->azureSubscriptionId = $data['azureSubscriptionId'];
        }
        if (isset($data['azureTenantId'])) {
            $this->azureTenantId = $data['azureTenantId'];
        }
        if (isset($data['category'])) {
            $this->category = $data['category'];
        }
        if (isset($data['closedDateTime'])) {
            $this->closedDateTime = is_string($data['closedDateTime']) ? new \DateTimeImmutable($data['closedDateTime']) : $data['closedDateTime'];
        }
        if (isset($data['cloudAppStates'])) {
            $this->cloudAppStates = $data['cloudAppStates'];
        }
        if (isset($data['comments'])) {
            $this->comments = $data['comments'];
        }
        if (isset($data['confidence'])) {
            $this->confidence = is_numeric($data['confidence']) ? (float)$data['confidence'] : $data['confidence'];
        }
        if (isset($data['createdDateTime'])) {
            $this->createdDateTime = is_string($data['createdDateTime']) ? new \DateTimeImmutable($data['createdDateTime']) : $data['createdDateTime'];
        }
        if (isset($data['description'])) {
            $this->description = $data['description'];
        }
        if (isset($data['detectionIds'])) {
            $this->detectionIds = $data['detectionIds'];
        }
        if (isset($data['eventDateTime'])) {
            $this->eventDateTime = is_string($data['eventDateTime']) ? new \DateTimeImmutable($data['eventDateTime']) : $data['eventDateTime'];
        }
        if (isset($data['feedback'])) {
            $this->feedback = is_string($data['feedback']) ? AlertFeedback::tryFrom($data['feedback']) : $data['feedback'];
        }
        if (isset($data['fileStates'])) {
            $this->fileStates = $data['fileStates'];
        }
        if (isset($data['historyStates'])) {
            $this->historyStates = $data['historyStates'];
        }
        if (isset($data['hostStates'])) {
            $this->hostStates = $data['hostStates'];
        }
        if (isset($data['incidentIds'])) {
            $this->incidentIds = $data['incidentIds'];
        }
        if (isset($data['investigationSecurityStates'])) {
            $this->investigationSecurityStates = $data['investigationSecurityStates'];
        }
        if (isset($data['lastEventDateTime'])) {
            $this->lastEventDateTime = is_string($data['lastEventDateTime']) ? new \DateTimeImmutable($data['lastEventDateTime']) : $data['lastEventDateTime'];
        }
        if (isset($data['lastModifiedDateTime'])) {
            $this->lastModifiedDateTime = is_string($data['lastModifiedDateTime']) ? new \DateTimeImmutable($data['lastModifiedDateTime']) : $data['lastModifiedDateTime'];
        }
        if (isset($data['malwareStates'])) {
            $this->malwareStates = $data['malwareStates'];
        }
        if (isset($data['messageSecurityStates'])) {
            $this->messageSecurityStates = $data['messageSecurityStates'];
        }
        if (isset($data['networkConnections'])) {
            $this->networkConnections = $data['networkConnections'];
        }
        if (isset($data['processes'])) {
            $this->processes = $data['processes'];
        }
        if (isset($data['recommendedActions'])) {
            $this->recommendedActions = $data['recommendedActions'];
        }
        if (isset($data['registryKeyStates'])) {
            $this->registryKeyStates = $data['registryKeyStates'];
        }
        if (isset($data['securityResources'])) {
            $this->securityResources = $data['securityResources'];
        }
        if (isset($data['severity'])) {
            $this->severity = is_string($data['severity']) ? AlertSeverity::tryFrom($data['severity']) : $data['severity'];
        }
        if (isset($data['sourceMaterials'])) {
            $this->sourceMaterials = $data['sourceMaterials'];
        }
        if (isset($data['status'])) {
            $this->status = is_string($data['status']) ? AlertStatus::tryFrom($data['status']) : $data['status'];
        }
        if (isset($data['tags'])) {
            $this->tags = $data['tags'];
        }
        if (isset($data['title'])) {
            $this->title = $data['title'];
        }
        if (isset($data['triggers'])) {
            $this->triggers = $data['triggers'];
        }
        if (isset($data['uriClickSecurityStates'])) {
            $this->uriClickSecurityStates = $data['uriClickSecurityStates'];
        }
        if (isset($data['userStates'])) {
            $this->userStates = $data['userStates'];
        }
        if (isset($data['vendorInformation'])) {
            $this->vendorInformation = is_array($data['vendorInformation']) ? new SecurityVendorInformation($data['vendorInformation']) : $data['vendorInformation'];
        }
        if (isset($data['vulnerabilityStates'])) {
            $this->vulnerabilityStates = $data['vulnerabilityStates'];
        }
    }

    /**
     * Get raw data from API response
     * @return array<string, mixed>
     */
    public function getRaw(): array
    {
        return $this->rawData;
    }
}
