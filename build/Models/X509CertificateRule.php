...
class X509CertificateRule
{
    ...
    public function __construct(array $data = [])
    {
        ...
        if (isset($data['x509CertificateAuthenticationMode'])) {
            $this->x509CertificateAuthenticationMode = is_string($data['x509CertificateAuthenticationMode']) ? X509CertificateAuthenticationMode::tryFrom($data['x509CertificateAuthenticationMode']) : $data['x509CertificateAuthenticationMode'];
        }
        if (isset($data['x509CertificateRequiredAffinityLevel'])) {
            $this->x509CertificateRequiredAffinityLevel = is_string($data['x509CertificateRequiredAffinityLevel']) ? X509CertificateAffinityLevel::tryFrom($data['x509CertificateRequiredAffinityLevel']) : $data['x509CertificateRequiredAffinityLevel'];
        }
        if (isset($data['x509CertificateRuleType'])) {
            $this->x509CertificateRuleType = is_string($data['x509CertificateRuleType']) ? X509CertificateRuleType::tryFrom($data['x509CertificateRuleType']) : $data['x509CertificateRuleType'];
        }
    }
