<?php

declare(strict_types=1);

namespace ApeDevDe\MicrosoftGraphSdk\Models;

/**
 * InternalDomainFederation
 */
class InternalDomainFederation
{
    /**
     * URL of the endpoint used by active clients when authenticating with federated domains set up for single sign-on in Microsoft Entra ID. Corresponds to the ActiveLogOnUri property of the Set-EntraDomainFederationSettings PowerShell cmdlet.
     */
    private ?string $activeSignInUri;

    /**
     * Determines whether Microsoft Entra ID accepts the MFA performed by the federated IdP when a federated user accesses an application that is governed by a conditional access policy that requires MFA. The possible values are: acceptIfMfaDoneByFederatedIdp, enforceMfaByFederatedIdp, rejectMfaByFederatedIdp, unknownFutureValue. For more information, see federatedIdpMfaBehavior values.
     */
    private ?string $federatedIdpMfaBehavior;

    /**
     * If true, when SAML authentication requests are sent to the federated SAML IdP, Microsoft Entra ID will sign those requests using the OrgID signing key. If false (default), the SAML authentication requests sent to the federated IdP aren''t signed.
     */
    private ?bool $isSignedAuthenticationRequestRequired;

    /**
     * Fallback token signing certificate that can also be used to sign tokens, for example when the primary signing certificate expires. Formatted as Base64 encoded strings of the public portion of the federated IdP''s token signing certificate. Needs to be compatible with the X509Certificate2 class. Much like the signingCertificate, the nextSigningCertificate property is used if a rollover is required outside of the auto-rollover update, a new federation service is being set up, or if the new token signing certificate isn''t present in the federation properties after the federation service certificate has been updated.
     */
    private ?string $nextSigningCertificate;

    /**
     */
    private ?string $passwordResetUri;

    /**
     * Sets the preferred behavior for the sign-in prompt. The possible values are: translateToFreshPasswordAuthentication, nativeSupport, disabled, unknownFutureValue.
     */
    private ?string $promptLoginBehavior;

    /**
     * Provides status and timestamp of the last update of the signing certificate.
     */
    private ?string $signingCertificateUpdateStatus;

    /**
     * URI that clients are redirected to when they sign out of Microsoft Entra services. Corresponds to the LogOffUri property of the Set-EntraDomainFederationSettings PowerShell cmdlet.
     */
    private ?string $signOutUri;


    public function getActiveSignInUri(): ?string
    {
        return $this->activeSignInUri;
    }

    public function setActiveSignInUri(?string $activeSignInUri): self
    {
        $this->activeSignInUri = $activeSignInUri;
        return $this;
    }

    public function getFederatedIdpMfaBehavior(): ?string
    {
        return $this->federatedIdpMfaBehavior;
    }

    public function setFederatedIdpMfaBehavior(?string $federatedIdpMfaBehavior): self
    {
        $this->federatedIdpMfaBehavior = $federatedIdpMfaBehavior;
        return $this;
    }

    public function getIsSignedAuthenticationRequestRequired(): ?bool
    {
        return $this->isSignedAuthenticationRequestRequired;
    }

    public function setIsSignedAuthenticationRequestRequired(?bool $isSignedAuthenticationRequestRequired): self
    {
        $this->isSignedAuthenticationRequestRequired = $isSignedAuthenticationRequestRequired;
        return $this;
    }

    public function getNextSigningCertificate(): ?string
    {
        return $this->nextSigningCertificate;
    }

    public function setNextSigningCertificate(?string $nextSigningCertificate): self
    {
        $this->nextSigningCertificate = $nextSigningCertificate;
        return $this;
    }

    public function getPasswordResetUri(): ?string
    {
        return $this->passwordResetUri;
    }

    public function setPasswordResetUri(?string $passwordResetUri): self
    {
        $this->passwordResetUri = $passwordResetUri;
        return $this;
    }

    public function getPromptLoginBehavior(): ?string
    {
        return $this->promptLoginBehavior;
    }

    public function setPromptLoginBehavior(?string $promptLoginBehavior): self
    {
        $this->promptLoginBehavior = $promptLoginBehavior;
        return $this;
    }

    public function getSigningCertificateUpdateStatus(): ?string
    {
        return $this->signingCertificateUpdateStatus;
    }

    public function setSigningCertificateUpdateStatus(?string $signingCertificateUpdateStatus): self
    {
        $this->signingCertificateUpdateStatus = $signingCertificateUpdateStatus;
        return $this;
    }

    public function getSignOutUri(): ?string
    {
        return $this->signOutUri;
    }

    public function setSignOutUri(?string $signOutUri): self
    {
        $this->signOutUri = $signOutUri;
        return $this;
    }

}
