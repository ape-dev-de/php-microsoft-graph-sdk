...
/**
 * Tests for PermissionGrants collection
 */
class PermissionGrantsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->permissionGrants();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->permissionGrants()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->permissionGrants()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\PermissionGrantsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'PermissionGrantsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'PermissionGrantsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\PermissionGrantCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('PermissionGrantCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'PermissionGrantCollectionResponse should have variadic setValue parameter');
        }
    }
