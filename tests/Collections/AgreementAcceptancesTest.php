...
/**
 * Tests for AgreementAcceptances collection
 */
class AgreementAcceptancesTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->agreementAcceptances();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->agreementAcceptances()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->agreementAcceptances()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\AgreementAcceptancesQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'AgreementAcceptancesQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'AgreementAcceptancesQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\AgreementAcceptanceCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('AgreementAcceptanceCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'AgreementAcceptanceCollectionResponse should have variadic setValue parameter');
        }
    }
