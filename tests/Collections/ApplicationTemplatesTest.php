...
/**
 * Tests for ApplicationTemplates collection
 */
class ApplicationTemplatesTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->applicationTemplates();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->applicationTemplates()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->applicationTemplates()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\ApplicationTemplatesQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'ApplicationTemplatesQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'ApplicationTemplatesQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\ApplicationTemplateCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('ApplicationTemplateCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'ApplicationTemplateCollectionResponse should have variadic setValue parameter');
        }
    }

    public function testCanAccessInstantiateSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->applicationTemplates()->get(
            queryParameters: ['$top' => 1]
        ...
        $result = self::$client->applicationTemplates()->byId($itemId)->instantiate()->get();
