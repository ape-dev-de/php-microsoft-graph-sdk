...
/**
 * Tests for Contracts collection
 */
class ContractsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->contracts();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->contracts()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->contracts()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\ContractsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'ContractsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'ContractsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\ContractCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('ContractCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'ContractCollectionResponse should have variadic setValue parameter');
        }
    }
    ...
    public function testCanAccessCheckMemberGroupsSubpath(): void
    {
        ...
        $result = self::$client->contracts()->byId($itemId)->checkMemberGroups()->get();
    ...
    public function testCanAccessCheckMemberObjectsSubpath(): void
    {
        ...
        // Access subpath and get data
        $result = self::$client->contracts()->byId($itemId)->checkMemberObjects()->get();
        $this->assertNotNull($result);
    }
    ...
    public function testCanAccessGetMemberGroupsSubpath(): void
    {
        ...
        // Access subpath and get data
        $result = self::$client->contracts()->byId($itemId)->getMemberGroups()->get();
        $this->assertNotNull($result);
    }
