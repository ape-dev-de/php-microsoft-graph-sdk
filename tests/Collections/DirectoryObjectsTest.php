...
/**
 * Tests for DirectoryObjects collection
 */
class DirectoryObjectsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->directoryObjects();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->directoryObjects()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->directoryObjects()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\DirectoryObjectsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'DirectoryObjectsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'DirectoryObjectsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\DirectoryObjectCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('DirectoryObjectCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'DirectoryObjectCollectionResponse should have variadic setValue parameter');
        }
    }
