...
/**
 * Tests for GroupLifecyclePolicies collection
 */
class GroupLifecyclePoliciesTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->groupLifecyclePolicies();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->groupLifecyclePolicies()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->groupLifecyclePolicies()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\GroupLifecyclePoliciesQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'GroupLifecyclePoliciesQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'GroupLifecyclePoliciesQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\GroupLifecyclePolicieCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('GroupLifecyclePolicieCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'GroupLifecyclePolicieCollectionResponse should have variadic setValue parameter');
        }
    }
