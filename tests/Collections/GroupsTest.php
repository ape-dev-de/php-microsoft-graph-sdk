...
class GroupsTest extends GraphTestCase
{
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->groups()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\GroupsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'GroupsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'GroupsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\GroupCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('GroupCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'GroupCollectionResponse should have variadic setValue parameter');
        }
    }
