...
/**
 * Tests for Domains collection
 */
class DomainsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->domains();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->domains()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->domains()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\DomainsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'DomainsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'DomainsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\DomainCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('DomainCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'DomainCollectionResponse should have variadic setValue parameter');
        }
    }

    public function testCanAccessDomainNameReferencesSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->domains()->get(
            queryParameters: ['$top' => 1]
        ...
        $result = self::$client->domains()->byId($itemId)->domainNameReferences()->get();
        $this->assertNotNull($result);
    }

    public function testCanAccessFederationConfigurationSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->domains()->get(
            queryParameters: ['$top' => 1]
        ...
        // Access subpath and get data
        $result = self::$client->domains()->byId($itemId)->federationConfiguration()->get();
        $this->assertNotNull($result);
    }

    public function testCanAccessForceDeleteSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->domains()->get(
            queryParameters: ['$top' => 1]
        ...
        // Access subpath and get data
        $result = self::$client->domains()->byId($itemId)->forceDelete()->get();
        $this->assertNotNull($result);
    }
