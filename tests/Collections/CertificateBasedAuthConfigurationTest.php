...
/**
 * Tests for CertificateBasedAuthConfiguration collection
 */
class CertificateBasedAuthConfigurationTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->certificateBasedAuthConfiguration();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->certificateBasedAuthConfiguration()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->certificateBasedAuthConfiguration()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\CertificateBasedAuthConfigurationQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'CertificateBasedAuthConfigurationQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'CertificateBasedAuthConfigurationQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\CertificateBasedAuthConfigurationCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('CertificateBasedAuthConfigurationCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'CertificateBasedAuthConfigurationCollectionResponse should have variadic setValue parameter');
        }
    }
