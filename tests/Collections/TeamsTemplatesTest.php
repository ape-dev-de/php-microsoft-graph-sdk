...
/**
 * Tests for TeamsTemplates collection
 */
class TeamsTemplatesTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->teamsTemplates();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->teamsTemplates()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->teamsTemplates()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\TeamsTemplatesQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'TeamsTemplatesQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'TeamsTemplatesQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\TeamsTemplateCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('TeamsTemplateCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'TeamsTemplateCollectionResponse should have variadic setValue parameter');
        }
    }
