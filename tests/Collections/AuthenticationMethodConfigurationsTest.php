...
/**
 * Tests for AuthenticationMethodConfigurations collection
 */
class AuthenticationMethodConfigurationsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->authenticationMethodConfigurations();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->authenticationMethodConfigurations()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->authenticationMethodConfigurations()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\AuthenticationMethodConfigurationsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'AuthenticationMethodConfigurationsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'AuthenticationMethodConfigurationsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\AuthenticationMethodConfigurationCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('AuthenticationMethodConfigurationCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'AuthenticationMethodConfigurationCollectionResponse should have variadic setValue parameter');
        }
    }
