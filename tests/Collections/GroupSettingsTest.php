...
/**
 * Tests for GroupSettings collection
 */
class GroupSettingsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->groupSettings();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->groupSettings()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->groupSettings()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\GroupSettingsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'GroupSettingsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'GroupSettingsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\GroupSettingCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('GroupSettingCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'GroupSettingCollectionResponse should have variadic setValue parameter');
        }
    }
