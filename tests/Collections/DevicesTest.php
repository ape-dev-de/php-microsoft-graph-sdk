...
/**
 * Tests for Devices collection
 */
class DevicesTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->devices();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->devices()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->devices()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\DevicesQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'DevicesQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'DevicesQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\DeviceCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('DeviceCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'DeviceCollectionResponse should have variadic setValue parameter');
        }
    }
    ...
    public function testCanAccessExtensionsSubpath(): void
    {
        ...
        $result = self::$client->devices()->byId($itemId)->extensions()->get();
        $this->assertNotNull($result);
    }

    public function testCanAccessMemberOfSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->devices()->get(
            queryParameters: ['$top' => 1]
        ...
        // Access subpath and get data
        $result = self::$client->devices()->byId($itemId)->memberOf()->get();
        $this->assertNotNull($result);
    }
    ...
    public function testCanAccessCheckMemberGroupsSubpath(): void
    {
        ...
        // Access subpath and get data
        $result = self::$client->devices()->byId($itemId)->checkMemberGroups()->get();
        $this->assertNotNull($result);
    }
