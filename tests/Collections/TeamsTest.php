...
class TeamsTest extends GraphTestCase
{
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->teams()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\TeamsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'TeamsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'TeamsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\TeamCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('TeamCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'TeamCollectionResponse should have variadic setValue parameter');
        }
    }

    public function testCanAccessAllChannelsSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->teams()->get(
            queryParameters: ['$top' => 1]
        ...
        $result = self::$client->teams()->byId($itemId)->allChannels()->get();
        $this->assertNotNull($result);
    }

    public function testCanAccessChannelsSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->teams()->get(
            queryParameters: ['$top' => 1]
        ...
        // Access subpath and get data
        $result = self::$client->teams()->byId($itemId)->channels()->get();
        $this->assertNotNull($result);
    }

    public function testCanAccessGroupSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->teams()->get(
            queryParameters: ['$top' => 1]
        ...
        // Access subpath and get data
        $result = self::$client->teams()->byId($itemId)->group()->get();
        $this->assertNotNull($result);
    }

    public function testCanAccessGroupServiceProvisioningErrorsNestedSubpath(): void
    {
        // Get first item from collection
        $collection = self::$client->teams()->get(
            queryParameters: ['$top' => 1]
        ...
        $result = self::$client->teams()->byId($itemId)->group()->serviceProvisioningErrors()->get();
        $this->assertNotNull($result);
    }
    ...
    public function testCanAccessMembersAddNestedSubpath(): void
    {
        ...
        // Access nested subpath and get data
        $result = self::$client->teams()->byId($itemId)->members()->add()->get();
        $this->assertNotNull($result);
    }
