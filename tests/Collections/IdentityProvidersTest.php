...
/**
 * Tests for IdentityProviders collection
 */
class IdentityProvidersTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->identityProviders();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->identityProviders()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->identityProviders()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\IdentityProvidersQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'IdentityProvidersQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'IdentityProvidersQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\IdentityProviderCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('IdentityProviderCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'IdentityProviderCollectionResponse should have variadic setValue parameter');
        }
    }
