...
/**
 * Tests for Contacts collection
 */
class ContactsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->contacts();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->contacts()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->contacts()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\ContactsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'ContactsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'ContactsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\ContactCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('ContactCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'ContactCollectionResponse should have variadic setValue parameter');
        }
    }
