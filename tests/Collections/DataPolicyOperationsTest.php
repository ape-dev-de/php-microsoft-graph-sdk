...
/**
 * Tests for DataPolicyOperations collection
 */
class DataPolicyOperationsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->dataPolicyOperations();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->dataPolicyOperations()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->dataPolicyOperations()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\DataPolicyOperationsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'DataPolicyOperationsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'DataPolicyOperationsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\DataPolicyOperationCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('DataPolicyOperationCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'DataPolicyOperationCollectionResponse should have variadic setValue parameter');
        }
    }
