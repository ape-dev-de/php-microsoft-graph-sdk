...
/**
 * Tests for Invitations collection
 */
class InvitationsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->invitations();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->invitations()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->invitations()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\InvitationsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'InvitationsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'InvitationsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\InvitationCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('InvitationCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'InvitationCollectionResponse should have variadic setValue parameter');
        }
    }
