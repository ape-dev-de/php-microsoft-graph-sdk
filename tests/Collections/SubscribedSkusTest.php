...
/**
 * Tests for SubscribedSkus collection
 */
class SubscribedSkusTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->subscribedSkus();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->subscribedSkus()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->subscribedSkus()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\SubscribedSkusQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'SubscribedSkusQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'SubscribedSkusQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\SubscribedSkuCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('SubscribedSkuCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'SubscribedSkuCollectionResponse should have variadic setValue parameter');
        }
    }
