...
/**
 * Tests for ScopedRoleMemberships collection
 */
class ScopedRoleMembershipsTest extends GraphTestCase
{
    public function testCanAccessBuilder(): void
    {
        $builder = self::$client->scopedRoleMemberships();
        $this->assertNotNull($builder);
    }

    public function testCanGetCollection(): void
    {
        $collection = self::$client->scopedRoleMemberships()->get();
    ...
    public function testCanGetItemById(): void
    {
        ...
        // Get item by ID
        $fetchedItem = self::$client->scopedRoleMemberships()->byId($itemId)->get();
    ...
    {
        $queryOptionsClass = 'ApeDevDe\\MicrosoftGraphSdk\\QueryOptions\\ScopedRoleMembershipsQueryOptions';
        ...
        $this->assertNotEmpty($constants, 'ScopedRoleMembershipsQueryOptions should have field constants');
        ...
        $this->assertNotEmpty($fieldConstants, 'ScopedRoleMembershipsQueryOptions should have FIELD_* constants');
    }

    public function testCollectionResponseHasTypeSafety(): void
    {
        $collectionClass = 'ApeDevDe\\MicrosoftGraphSdk\\Models\\ScopedRoleMembershipCollectionResponse';
        
        if (!class_exists($collectionClass)) {
            $this->markTestSkipped('ScopedRoleMembershipCollectionResponse does not exist');
        }
        ...
        if (count($parameters) > 0) {
            $this->assertTrue($parameters[0]->isVariadic(), 'ScopedRoleMembershipCollectionResponse should have variadic setValue parameter');
        }
    }
